let knygos = {
  ...
  // 1.2 Išvesti knygų kategorijas su pigiausiomis knygomis.
  ...
  for(i in knygos){
      ...
      console.log("\n")  
    for(j in knygos[i]){
        if(knygos[i][j].price < suma){
            suma = knygos[i][j].price
        }
    }
    for(k in knygos[i]){
        for(l in knygos[i][k]){
            if(knygos[i][k].price == suma){
                console.log(`${l}: ${knygos[i][k][l]}`);
            }       
        }
    }
    console.log("\n")  
